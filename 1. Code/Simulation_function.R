...
# Tune the lasso to give sensible TPR
...
# Estimate sigma in the high-dimension -- RCV
estimateSigma2 <- function(x, y, true_vars,lamb, intercept=FALSE, standardize=TRUE){
    # assume 'true_vars' is a vector containing the indices of true relevant variables
    n = dim(x)[1]
...
COV2 = function(n,p,beta0,rho,B,f,level,sigma,true_vars=NULL,lamb=NULL,estimateVar = FALSE){
  ...
  if(estimateVar){
    ...
    if(p>n){
      TPR = c()  ## to store the TPR when estimating sigma in the high-dim
    }
  }
  ...
  for(b in 1:B){
    ...
    if(sum(S_R) != 0){
      ...
      if(estimateVar){ ## when sigma is unknown, use the estimator of sigma
