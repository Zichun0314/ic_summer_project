# Table -- (CI Length, Coverage probability, FCR)
...
result_var_cov = function(...){
  
  table_var_cov = function(input){
    
    coverage = input$COV_R_HD
    
    table = data.frame(coverage=coverage)
    count <<- count + 1
    
    var_cov_table = table %>% 
      summarise(!! "Mean" := format(round(mean(coverage)*100,2), nsmall = 1),
                !! "Variance" := format((mean(coverage)*(1-mean(coverage))/25000), nsmall = 1))
    
    rownames(var_cov_table) = "Coverage probability"
    return(var_cov_table)
  }
  
  count = 0
  table_R = lapply(...,table_var_cov)
...
result_plot = function(...){
  ...
  tmp = df_R %>% split(.$beta)%>%map(function(x){
    ggplot(data = x , aes(x=type, y=length, fill=type))+
      ...
      theme(strip.text.x = element_text(margin = margin(b = 0, t = 0), face =  "bold.italic",size = 12),
...
# Table for plotting-- Coverage probability
plot_cov = function(...){
  ...
  table_R2 = Reduce(function(...) merge(..., by="beta == 0"),table_R2)
  combined <- table_R2 %>%
    pivot_longer(cols = starts_with("Mean coverage"), names_to = "sigma", values_to = "Mean coverage") %>%
    mutate(sigma = ifelse(sigma == "Mean coverage.x", "Estimated", "Fixed"))
